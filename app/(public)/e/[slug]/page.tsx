      ...
      <div className="relative flex flex-col lg:flex-row lg:h-screen">
        ...
        <div className="w-full lg:w-[60%] p-6 pt-6 sm:pt-16 lg:p-12 lg:pt-16 lg:overflow-y-auto lg:h-screen">
          <div className="max-w-lg mx-auto w-full space-y-8">
            ...
            {hasMaxSpots && (
              <div className="flex items-start gap-3">
                <div className={`w-10 h-10 rounded-xl ${isFull ? 'bg-zinc-700/30 border-zinc-700/50' : 'bg-amber-500/10 border-amber-500/20'} border flex items-center justify-center shrink-0`}>
                  <svg className={`w-5 h-5 ${isFull ? 'text-zinc-400' : 'text-amber-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                  </svg>
                </div>
                <div>
                  <p className={`font-medium ${isFull ? 'text-zinc-300' : 'text-zinc-200'}`}>
                    {isFull ? 'Event Full' : `${spotsLeft} spots left`}
                  </p>
                  <p className="text-zinc-500 text-sm mt-0.5">
                    {hasPrice ? `${effectiveAttendeeCount} paid` : `${effectiveAttendeeCount} registered`}
                  </p>
                </div>
              </div>
            )}
